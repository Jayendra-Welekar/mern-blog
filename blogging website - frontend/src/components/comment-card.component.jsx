...
const CommentCard = ({ index, leftVal, commentData })=>{
    ...
    let {blog, setBlog, blog: {comments, activity, activity: {total_parent_comments}, comments: {results: commentsArr}, author: { personal_info: { username: blog_author } }}, setTotalParentCommentsLoaded} = useContext(BlogContext)
        ...
        if(commentsArr[currentIndex].children.length) {
            ...
            .then(({ data: {replies} })=>{
                ...
                for(let i = 0; i < replies.length; i++){
                    ...
                    console.log(replies[i].childrenLevel)
    ...
    return (
        <div className="w-full " style={{ paddingLeft: `${(leftVal)*50}` + "px" }}>
            <Toaster />
            <div className="my-5 p-6 rounded-md border border-grey">
                ...
                <p className="font-gelassio text-xl ml-3">{ comment }</p>
...

                {
                    isReplying ? 
                    <div className="mt-8">
                        <CommentField index={index} action="reply" replyingTo={_id} setReplying={setReplying} />
                    </div> : ""
                }
            </div>
